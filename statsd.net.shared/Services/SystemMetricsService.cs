...
namespace statsd.net.shared.Services
{
  ...
  public class SystemMetricsService : ISystemMetricsService
  {
    ...
    private ConcurrentDictionary<MetricInfo, double> _metrics;
    ...
    {
      _metrics.AddOrUpdate(new MetricInfo(name), quantity, (key, input) => { return input + quantity; });
    }

    public void LogGauge(string name, int value)
    {
      _metrics.AddOrUpdate(new MetricInfo(name), value, (key, input) => { return value; });
    }
